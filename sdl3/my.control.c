...
static int my_control_start(ui_control_t *control);
static int my_control_stop(ui_control_t *control);
static int my_control_ioctl(ui_control_t *control, int type, void *data);
static int my_control_task(void *cookie);
...
static ui_control_t						live_control =
										{
											.name		= "live1.control",
											.priority	= 1,
											.start		= my_control_start,
											.stop		= my_control_stop,
											.ioctl		= my_control_ioctl,
...
int my_control_init(const char *name)
{
...
int my_control_init(const char *name)
{
	...
	control=malloc(sizeof(ui_control_t));
	memset(control,0,sizeof(ui_control_t));
	control->priority = 1;
	control->start = my_control_start;
	control->stop = my_control_stop;
	control->ioctl = my_control_ioctl;
